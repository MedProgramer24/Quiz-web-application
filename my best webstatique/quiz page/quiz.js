...
let aficherquestion = document.getElementById("question");
    ...
    for (let lang in languages) {
        if (lang == lange) {
            for (let i = 0; i < languages[lang].quiz_questions.length; i++) {
                let question = languages[lang].quiz_questions[step];
                for (let item in question) {
                    if (item == "question") {
                        aficherquestion.textContent = question.question;
                    }
                }
                let ind = 0;
                buttons.forEach((button) => {
                    button.textContent = question.options[ind];
                    ind++
                })
            }
...
buttons.forEach((button) => {
    button.addEventListener('click', () => {
        if (!answerschoosen.includes(button.textContent)) {
            let obj = [
                button.textContent
            ]
            answerschoosen.push(obj);
            let res = isgreen();
            if (res == false) {
...
submit.addEventListener('click', () => {
    if (step == 20) {
        stopTimer()
        let timeLeft = document.getElementById('affTime').textContent;
        localStorage.setItem('timeLeft', JSON.stringify(timeLeft));
        localStorage.setItem('answerschoosen', JSON.stringify(answerschoosen));
        localStorage.setItem('language', JSON.stringify(lange));
        window.location.href = "file:///C:/Users/LENOVO/Desktop/my%20best%20webstatique/statistics%20folder/statistics.html"
    }
    if (step < languages[lange].quiz_questions.length) {

        nextquestion(step);
        questionNumber.textContent = step + 1;
        step++;buttons.forEach((button) => {
            button.style.backgroundColor = "#E00B2E";
        })
...
function updateTimer() {
    ...
    document.getElementById('affTime').textContent = display;
}
function stopTimer() {
    clearInterval(timer);
}
